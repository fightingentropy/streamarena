...
ssh -i "$SSH_KEY" -o BatchMode=yes -o ConnectTimeout=10 "$MINI_HOST" \
  "REMOTE_APP='$REMOTE_APP' DISK_MAX_PERCENT='$DISK_MAX_PERCENT' DISK_MIN_FREE_GB='$DISK_MIN_FREE_GB' bash -s" <<'REMOTE'
set -euo pipefail
...
chmod 700 "$bin_dir/netflix-rotate-logs" "$bin_dir/netflix-disk-monitor"
