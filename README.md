...
- CineJoy offers **Lisbon**, **Nebula**, and **Solara** native HLS sources. Only Lisbon participates in automatic fallback; Nebula and Solara are manual choices sharing the same provider budget, cache, health, and admin controls (`embed:cinejoy:enabled` / `embed:cinejoy:rank`). CineJoy Nebula is separate from the configured NebulaStreams addon.
...
- The iPhone app plays CineJoy through AVPlayer with the same full adaptive quality ladder, preserving hardware HEVC/HDR playback without the strip-proxy/VLC workaround used for disguised transport streams.
