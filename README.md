...
- CineJoy adds three distinct source choices: **CineJoy Lisbon**, **CineJoy Nebula**, and **CineJoy Solara**. Its [public server list](https://api.shegu.st/servers) was checked on 2026-09-04. Observed delivery hosts are `info.movieboxnoob.cc`, `nebula.bright67.online`, and `lol.movieboxnoob.cc`, respectively; the Nebula alias is separate from the configured NebulaStreams addon. Athens/Joy/Castle had no result for the sampled movie, Sakura is anime-series-only, and Canaias returned file/embed identifiers, so those unverified paths are not advertised as native HLS sources.
- CineJoy uses `scripts/resolve-cinejoy-hls.mjs` (deployed as `bin/resolve-cinejoy-hls.mjs`) and the existing Playwright installation. The helper allows only essential page/API requests, pins one server, captures its master URL without downloading media, and closes Chromium. The backend validates `#EXTM3U` with the required CineJoy referer before signing the existing native HLS proxy path. No CineJoy account, copied API key, third-party decrypt service, or hardcoded expiring stream link is needed. Only Lisbon participates in automatic fallback; Nebula/Solara are manually selectable to avoid launching multiple browsers during the fallback race. Existing source caching, health, concurrency budgets, and the admin `embed:cinejoy:enabled` / `embed:cinejoy:rank` controls apply. The discovered CDN endpoints require the provider referer, so media stays on the established proxy path instead of assuming browser-direct compatibility.
