...
pub use admin::record_health_sample;
use admin::{
    admin_activity_handler, admin_delete_feedback_handler, admin_delete_user_handler,
    admin_feedback_handler, admin_feedback_image_handler, admin_growth_handler,
    admin_health_handler, admin_health_history_handler, admin_live_top_handler,
    admin_overview_handler, admin_provider_add_handler, admin_provider_remove_handler,
    admin_provider_set_handler, admin_provider_test_handler, admin_providers_handler,
    admin_reset_password_handler, admin_set_admin_handler, admin_set_disabled_handler,
    admin_user_detail_handler, admin_users_handler,
};
#[cfg(test)]
...
const REAL_DEBRID_ENABLED_PREF_KEY: &str = "streamarena-real-debrid-enabled";
...
mod tests {
    ...
    #[test]
    fn real_debrid_enablement_is_explicit_with_legacy_compatibility() {
        assert!(effective_real_debrid_enabled(None, true));
        assert!(!effective_real_debrid_enabled(None, false));
        assert!(!effective_real_debrid_enabled(Some("1"), false));
        assert!(!effective_real_debrid_enabled(Some("0"), true));
        assert!(effective_real_debrid_enabled(Some("enabled"), true));
        assert!(normalize_bool_json_value(&serde_json::json!(true)));
        assert!(normalize_bool_json_value(&serde_json::json!(1)));
        assert!(!normalize_bool_json_value(&serde_json::json!("off")));

        let missing = plan_real_debrid_update(false, None, Some(true))
            .expect_err("enabling without a token must fail before writes");
        assert_eq!(missing.status(), axum::http::StatusCode::FAILED_DEPENDENCY);
        assert_eq!(
            plan_real_debrid_update(true, None, Some(false)).expect("disable keeps token"),
            (true, false)
        );
        assert_eq!(
            plan_real_debrid_update(true, Some(true), Some(true)).expect("replace and enable"),
            (true, true)
        );
    }

    #[test]
    fn real_debrid_tokens_are_validated_and_masked_without_disclosure() {
        let token = "abcdefghijklmnop1234567890";
        assert!(is_valid_real_debrid_api_key(token));
        assert!(!is_valid_real_debrid_api_key("short"));
        assert!(!is_valid_real_debrid_api_key("token with whitespace"));
        assert!(!is_valid_real_debrid_api_key("éééééééééééééééé"));
        let masked = mask_real_debrid_api_key(token);
        assert_eq!(masked, "abcd****7890");
